...
namespace GameFrame
{
    public class IconManager : GameController<IconManager>
    {
        ...
        NTexture LoadSync(string url)
        {
            ...
            var record = IconTable.Instance.Get(id);
            if(record != null)
            {
                var raw = ResourceProcedure.Instance.Load<Texture2D>("UI/Icons/tubiao" + record.TextureID.ToString());
                if (raw != null)
                {
                    NTexture ret = new NTexture(raw, new Rect(record.PX, record.PY, record.Width, record.Height));
                    return ret;
                }
            }
            return null;
