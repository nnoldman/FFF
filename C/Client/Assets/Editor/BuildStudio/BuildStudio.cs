...
public class BuildStudio : EditorWindow {
    public const string StreamingABFloder = "Assets/StreamingAssets/" + Frame.FrameOption.Files.abFloder;
    ...
    static Frame.GameVersion mCurrentVersion = new Frame.GameVersion();
    ...
    static string GetIndexFileName() {
        string path = Path.Combine(GetServerFloder(), Frame.FrameOption.Files.cdnIndexFile);
        return path;
    }
    ...
    static Frame.AssetBundleIndex GetAssetBundleInfoListFromFile() {
        string fileFullName = GetIndexFileName();
        if (File.Exists(fileFullName)) {
            string content = File.ReadAllText(fileFullName);
            return JsonUtility.FromJson<Frame.AssetBundleIndex>(content);
        }
        return null;
    }
